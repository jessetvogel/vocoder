function effectPitch(div) {
	// Create pitch effect
	command('ADD pitch myPitch');
	...
	div
		...
		.append($('<div>').addClass('label').text('P I T C H'))
		.append($('<div>').addClass('slideContainer')
			.append($('<div>').addClass('strip'))
			.append($('<div>').addClass('slide')) // slider for pitch
			.append($('<span>').addClass('labelTop').text('+12')).append($('<span>').addClass('labelBottom').text('-12'))
		);
...
//		value = Math.round(value*12.0)/12.0;
		command('UPDATE myPitch shift ' + Math.pow(2.0, value));
	});
